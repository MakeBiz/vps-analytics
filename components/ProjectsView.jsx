...
const COLORS = ['#03c9ff', '#21ddb2', '#03c9ff', '#ff526f', '#f4bd58', '#7f9dbb', '#b98cc4', '#5bb0b0'];
...
export default function ProjectsView({ initial, spend }) {
  ...
  const campRow = (c) => (
    <tr key={c.id} style={c.status === 'excluded' ? { opacity: 0.5 } : undefined}>
      <td>
        {c.is_new ? <span style={{ ...stStyle.active, background: 'rgba(8,183,242,.18)', color: 'var(--brass)', borderColor: 'rgba(8,183,242,.4)', fontSize: 11, padding: '2px 8px', borderRadius: 20, border: '1px solid', marginRight: 6 }}>новая</span> : null}
        {c.name}
  ...
  const saveBar = (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, justifyContent: 'flex-end' }}>
      ...
      <button onClick={save} disabled={saving} style={{ background: dirty ? 'linear-gradient(180deg,#03c9ff,#068bdd)' : 'var(--panel-2)', border: '1px solid ' + (dirty ? '#068bdd' : 'var(--line)'), color: dirty ? '#20170a' : 'var(--text)', fontWeight: 600, borderRadius: 9, padding: '9px 18px', cursor: 'pointer', fontSize: 13.5, opacity: saving ? 0.6 : 1 }}>
