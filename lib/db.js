...
-- Проекты для разнесения рекламного расхода. Два уровня (kind): провайдер и сайт.
-- Расход кампаний Директа раскидывается по проектам, чтобы считать net-роялти.
create table if not exists ad_projects (
  id serial primary key,
  slug text unique not null,
  name text not null,
  kind text not null default 'provider',   -- 'provider' | 'site'
  roy_key text not null default '',         -- ключ в royalties.json (tw/avps/ish/aeza) для дохода, если провайдер
  archived boolean not null default false,
  sort int not null default 0,
  created_at timestamptz not null default now()
);

-- Настройки кампаний Директа (правятся из кабинета). Кампании тянутся из снимка
-- автоматически; для новой строки этой таблицы нет — слой projects создаёт её со
-- смысловыми значениями по умолчанию. alloc: slug проекта | 'split' | 'none'.
create table if not exists ad_campaign_settings (
  ...
  provider_alloc text not null default 'split',
  site_alloc text not null default 'split',
  updated_at timestamptz not null default now()
);
-- Единая привязка кампании (одна на кампанию): slug проекта (провайдер ИЛИ сайт) или 'split'.
alter table ad_campaign_settings add column if not exists alloc text;

-- Направление бизнеса у площадки: vps (каталоги и партнёрки хостингов) или
-- company (сайты MakeBiz: внедрение, CRM, ИИ). Один фильтр в шапке панели
-- переключает все отчёты между направлениями и общим видом.
alter table sites add column if not exists line text not null default 'vps';
...
const SEED_SITES = [
  ...
  // Перезапуск как англоязычный каталог (2026-08-29): свежий ключ, чтобы старые
  // мёртвые сессии закрытого сайта не примешивались к новой истории.
  ['serverselection-online', 'ServerSelection', 'serverselection.online'],
  ['podborvps', 'ПодборVPS', 'podborvps.ru'],
  // Сайт компании (не VPS): внедрение/CRM/AI. Общая аналитика — визиты, гео,
  // источники, воронки, цель «заявка/форма». VPS-специфика к нему не относится.
  ['makebiz-life', 'MakeBiz', 'makebiz.life'],
  // Корпоративный сайт компании (12.09.2026). Внедрение Битрикс24, AI-агенты,
  // Vector, IntDoc. Направление company, как и makebiz.life
  ['makebiz-com', 'MakeBiz Technologies', 'makebiztechnologies.com'],
];
...
// Проекты по умолчанию: провайдеры-партнёры (с ключом дохода в royalties.json)
// и сайты. Пользователь дальше правит их из кабинета. [slug, name, kind, roy_key, sort]
const SEED_AD_PROJECTS = [
  ['timeweb', 'Timeweb', 'provider', 'tw', 10],
  ['adminvps', 'AdminVPS', 'provider', 'avps', 20],
  ['aeza', 'Aeza', 'provider', 'aeza', 30],
  ['ishosting', 'is*hosting', 'provider', 'ish', 40],
  ['hostman', 'Hostman', 'provider', 'hostman', 50],
  ['podborvps', 'ПодборVPS', 'site', '', 110],
  ['servercalc-ru', 'ServerCalc.ru', 'site', '', 120],
  ['servercalc-com', 'ServerCalc.com', 'site', '', 130],
  ['serverselection-online', 'ServerSelection', 'site', '', 140],
];
...
async function seed(p) {
  ...
  for (const [slug, name, kind, roy_key, sort] of SEED_AD_PROJECTS) {
    await p.query(
      'insert into ad_projects (slug, name, kind, roy_key, sort) values ($1,$2,$3,$4,$5) on conflict (slug) do nothing',
      [slug, name, kind, roy_key, sort]
    );
  }
  // Сайты компании (не VPS-каталоги): помечаем направление, чтобы фильтр в
  // шапке разводил их с VPS-площадками.
  await p.query("update sites set line = 'company' where key in ('makebiz-life','makebiz-com') and line <> 'company'");
