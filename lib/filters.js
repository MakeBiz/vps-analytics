...
// Направления бизнеса. У каждой площадки в базе есть признак line, и этот
// фильтр переключает панель между «всё вместе» и конкретным направлением.
export const LINES = [
  ['', 'Все направления'],
  ['vps', 'VPS'],
  ['company', 'MakeBiz'],
...
// Разрез времени на графиках. Если не выбран руками — подбираем по длине периода.
export const GRANS = [
  ['day', 'День'],
  ['week', 'Неделя'],
  ['month', 'Месяц'],
];
export const GRAN_RU = { day: 'дни', week: 'недели', month: 'месяцы' };
...
export function parseFilters(sp = {}) {
  ...
  const line = ['vps', 'company'].includes(g('line')) ? g('line') : '';
  const granPick = GRANS.some(([k]) => k === g('g')) ? g('g') : '';
  const gran = granPick || autoGran(days);

  return {
    tz, range, from, to, site, bots, provider, q, source, days, line, gran,
    granAuto: !granPick,
...
export function qs(f, extra = {}) {
  const o = {
    tz: f.tz, site: f.site, line: f.line || '', bots: f.bots ? '1' : '',
    src: f.source || '', g: f.granAuto ? '' : f.gran, ...extra,
  };
