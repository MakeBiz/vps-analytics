...
export function guessDefaults(c) {
  ...
  if (id === '713238736' || /solara|\bcrm\b|support|битрикс|bitrix/.test(name)) {
    return { status: 'excluded', in_budget: false, provider_alloc: 'none', site_alloc: 'none' };
  }
  let provider = 'split';
  let site = 'split';
  let known = VPS_ALLOW.has(id);
  if (SITE_CAMP[id]) { site = SITE_CAMP[id]; provider = 'split'; known = true; }
  else if (/adminvps/.test(name)) { provider = 'adminvps'; known = true; }
  else if (/aeza/.test(name)) { provider = 'aeza'; known = true; }
  else if (/ishosting|is\*hosting/.test(name)) { provider = 'ishosting'; known = true; }
  else if (/timeweb|tw cloud|tw мск|tw спб/.test(name)) { provider = 'timeweb'; known = true; }
  if (known) return { status: 'active', in_budget: true, provider_alloc: provider, site_alloc: site };
  return { status: 'archived', in_budget: false, provider_alloc: 'split', site_alloc: 'split' };
}
...
export async function loadProjectBoard() {
  ...
  for (const id of allIds) {
    ...
    await q(
      ...
      [id, nameOf[id] || '', d.status, d.in_budget, d.provider_alloc, d.site_alloc]
    );
    fresh.push(id);
  }
