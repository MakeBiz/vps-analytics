/**
 * Слой «Проекты и кампании»: чтение/сохранение конфигурации разнесения
 * рекламного расхода Директа по проектам (два уровня — провайдер и сайт) и
 * расчёт расхода по проектам + net-роялти (доход из royalties.json − расход).
 *
 * Кампании тянутся из снимка (data/marketing.json, секция direct.campaigns) —
 * их собирает коннектор автоматически. Настройки (статус/бюджет/привязка) живут
 * в БД и правятся из кабинета. Для новой кампании строки настроек ещё нет —
 * создаём её со смысловыми значениями по умолчанию (guessDefaults), чтобы кампания
 * появлялась сама и сразу разумно распределялась.
 */
import { q } from './db';
import { loadMarketing } from './marketing';
import { loadRoyalties } from './royalties';

const num = (v) => (Number.isFinite(Number(v)) ? Number(v) : 0);

// Агрегаторы: у них известен сайт, а провайдер — общий (делится на всех).
const SITE_CAMP = {
  '713245534': 'podborvps', '713793556': 'podborvps',
  '713332123': 'servercalc-ru', '713793989': 'servercalc-ru',
};

// Текущие согласованные VPS-кампании (как в lib/direct.js и коннекторе).
const VPS_ALLOW = new Set([
  '708098448', '706715098', '706716163', '712849076', '713792287', '713636116', '713885050',
  '708902123', '713897708', '713771451', '713886032', '713886156', '713775967',
  '713245534', '713793556', '713332123', '713793989',
]);

// Значения по умолчанию для новой/исторической кампании (пока пользователь не задал).
// Правило: известные VPS-кампании — активные и в бюджете; явные не-VPS (Solara, CRM,
// Битрикс) — исключены; всё прочее историческое — паркуем в архив и НЕ в бюджет,
// чтобы ничего не считалось, пока Антон сам не отметит галку.
export function guessDefaults(c) {
  const id = String(c.id);
  const name = String(c.name || '').toLowerCase();
  if (id === '713238736' || /solara|\bcrm\b|support|битрикс|bitrix/.test(name)) {
    return { status: 'excluded', in_budget: false, alloc: 'split' };
  }
  let alloc = 'split';
  let known = VPS_ALLOW.has(id);
  if (SITE_CAMP[id]) { alloc = SITE_CAMP[id]; known = true; }               // агрегатор → сайт
  else if (/adminvps/.test(name)) { alloc = 'adminvps'; known = true; }
  else if (/aeza/.test(name)) { alloc = 'aeza'; known = true; }
  else if (/ishosting|is\*hosting/.test(name)) { alloc = 'ishosting'; known = true; }
  else if (/timeweb|tw cloud|tw мск|tw спб/.test(name)) { alloc = 'timeweb'; known = true; }
  if (known) return { status: 'active', in_budget: true, alloc };
  return { status: 'archived', in_budget: false, alloc: 'split' };
}

// Миграция старой двухуровневой привязки в единую: провайдер важнее, потом сайт.
function resolveAlloc(providerAlloc, siteAlloc, provSet, siteSet) {
  if (provSet.has(providerAlloc)) return providerAlloc;
  if (siteSet.has(siteAlloc)) return siteAlloc;
  if (siteSet.has(providerAlloc)) return providerAlloc;
  if (provSet.has(siteAlloc)) return siteAlloc;
  return 'split';
}

export async function getProjects() {
  return q('select id, slug, name, kind, roy_key, archived, sort from ad_projects order by kind desc, sort, id');
}

// Разнесение расхода. Единая привязка (alloc): конкретный провайдер → весь расход
// ему; конкретный сайт → весь расход этому сайту; 'split' → поровну. В разрезе
// ПРОВАЙДЕРОВ расход сайтовых и общих кампаний делится поровну на провайдеров; в
// разрезе САЙТОВ расход провайдерских и общих делится поровну на сайты.
export function attribute(campaigns, settingsMap, projects) {
  const provs = projects.filter((p) => p.kind === 'provider' && !p.archived);
  const sites = projects.filter((p) => p.kind === 'site' && !p.archived);
  const provSet = new Set(provs.map((p) => p.slug));
  const siteSet = new Set(sites.map((p) => p.slug));
  const spendProv = {}; provs.forEach((p) => { spendProv[p.slug] = 0; });
  const spendSite = {}; sites.forEach((p) => { spendSite[p.slug] = 0; });
  let sharedProv = 0; let sharedSite = 0; let counted = 0; let excluded = 0;
  for (const c of campaigns) {
    const s = settingsMap[String(c.id)];
    const cost = num(c.cost);
    if (!s || s.status === 'excluded' || !s.in_budget) { if (s && s.status === 'excluded') excluded += cost; continue; }
    counted += cost;
    const a = s.alloc;
    if (provSet.has(a)) spendProv[a] += cost; else sharedProv += cost; // сайт/split → на провайдеров поровну
    if (siteSet.has(a)) spendSite[a] += cost; else sharedSite += cost; // провайдер/split → на сайты поровну
  }
  const perProv = provs.length ? sharedProv / provs.length : 0;
  provs.forEach((p) => { spendProv[p.slug] += perProv; });
  const perSite = sites.length ? sharedSite / sites.length : 0;
  sites.forEach((p) => { spendSite[p.slug] += perSite; });
  return { spendProv, spendSite, sharedProv, sharedSite, perProv, perSite, counted, excluded };
}

// Доход по провайдеру из royalties.json (tw/avps/ish/aeza → .total).
function revenueFor(royKey) {
  if (!royKey) return null;
  const R = loadRoyalties();
  const node = R && R[royKey];
  if (!node) return null;
  return num(node.total);
}

export async function loadProjectBoard() {
  const m = loadMarketing();
  const snapCamps = (m && m.direct && Array.isArray(m.direct.campaigns)) ? m.direct.campaigns : [];
  const spendDaily = (m && m.spendDaily && typeof m.spendDaily === 'object') ? m.spendDaily : {};
  const since = (m && m.spendSince) || '2026-02-01';

  // Кумулятивный расход по кампании = сумма всех дней (с 01.02.2026). Фолбэк, пока
  // backfill не прошёл, — расход из 30-дневного снимка. Имена — из истории/снимка.
  const cumCost = {}; const nameOf = {};
  for (const [id, node] of Object.entries(spendDaily)) {
    const days = (node && node.days) || {};
    cumCost[id] = Object.values(days).reduce((s, v) => s + num(v), 0);
    if (node && node.name) nameOf[id] = node.name;
  }
  for (const c of snapCamps) {
    const id = String(c.id);
    if (!(id in cumCost)) cumCost[id] = num(c.cost);
    if (!nameOf[id]) nameOf[id] = c.name || '';
  }
  const present30 = new Set(snapCamps.map((c) => String(c.id)));
  const allIds = new Set([...Object.keys(cumCost), ...present30]);

  // «новая» — если первый расход по кампании был за последние 7 дней (по spendDaily)
  const weekAgo = new Date(Date.now() - 7 * 86400000).toISOString().slice(0, 10);
  const firstSeen = {};
  for (const [id, node] of Object.entries(spendDaily)) {
    const ds = Object.keys((node && node.days) || {}).sort();
    if (ds.length) firstSeen[id] = ds[0];
  }

  const projects = await getProjects();
  const provSet = new Set(projects.filter((p) => p.kind === 'provider').map((p) => p.slug));
  const siteSet = new Set(projects.filter((p) => p.kind === 'site').map((p) => p.slug));

  const SEL = 'select campaign_id, name, status, in_budget, provider_alloc, site_alloc, alloc from ad_campaign_settings';
  let rows = await q(SEL);
  const have = new Set(rows.map((r) => r.campaign_id));

  // Новым кампаниям (в т.ч. историческим из spendDaily) — строка настроек по умолчанию
  const fresh = [];
  for (const id of allIds) {
    if (have.has(id)) continue;
    const d = guessDefaults({ id, name: nameOf[id] || '' });
    await q(
      `insert into ad_campaign_settings (campaign_id, name, status, in_budget, alloc)
       values ($1,$2,$3,$4,$5) on conflict (campaign_id) do nothing`,
      [id, nameOf[id] || '', d.status, d.in_budget, d.alloc]
    );
    fresh.push(id);
  }
  if (fresh.length) rows = await q(SEL);

  // Миграция старых строк без единой привязки: alloc = из двухуровневой + запись обратно
  for (const r of rows) {
    if (!r.alloc) {
      r.alloc = resolveAlloc(r.provider_alloc, r.site_alloc, provSet, siteSet);
      await q('update ad_campaign_settings set alloc=$2 where campaign_id=$1', [r.campaign_id, r.alloc]);
    }
  }

  const settingsMap = {};
  for (const r of rows) settingsMap[r.campaign_id] = r;

  const list = rows
    .map((r) => ({
      id: r.campaign_id,
      name: nameOf[r.campaign_id] || r.name || r.campaign_id,
      cost: cumCost[r.campaign_id] || 0,
      status: r.status,
      in_budget: r.in_budget,
      alloc: r.alloc,
      is_new: !!(firstSeen[r.campaign_id] && firstSeen[r.campaign_id] >= weekAgo),
      present: present30.has(r.campaign_id),
    }))
    .sort((a, b) => b.cost - a.cost);

  const attr = attribute([...allIds].map((id) => ({ id, cost: cumCost[id] || 0 })), settingsMap, projects);

  // net по провайдерам-проектам, у которых задан roy_key
  const net = projects
    .filter((p) => p.kind === 'provider' && !p.archived && p.roy_key)
    .map((p) => {
      const revenue = revenueFor(p.roy_key);
      const spend = attr.spendProv[p.slug] || 0;
      return { slug: p.slug, name: p.name, revenue, spend, net: revenue == null ? null : revenue - spend };
    });

  // помесячная разбивка расхода (только «в бюджете») + итоги
  const inBudget = (id) => { const s = settingsMap[id]; return s && s.status !== 'excluded' && s.in_budget; };
  const byMonth = monthlySpend(spendDaily, inBudget);
  const totalRevenue = net.reduce((s, n) => s + (n.revenue || 0), 0);
  const totalNet = net.filter((n) => n.net != null).reduce((s, n) => s + n.net, 0);
  const totals = { budget: Math.round(attr.counted), revenue: Math.round(totalRevenue), net: Math.round(totalNet) };

  return { projects, campaigns: list, attr, net, totals, byMonth, generated: (m && m.generated) || null, since };
}

// Помесячный расход по дням (spendDaily) с фильтром «учитывать ли кампанию».
function monthlySpend(spendDaily, keep) {
  const mo = {};
  for (const [id, node] of Object.entries(spendDaily || {})) {
    if (keep && !keep(String(id))) continue;
    for (const [d, cost] of Object.entries((node && node.days) || {})) {
      const k = String(d).slice(0, 7);
      mo[k] = (mo[k] || 0) + num(cost);
    }
  }
  return Object.keys(mo).sort().map((k) => ({ m: k, cost: Math.round(mo[k]) }));
}

// Лёгкая сводка расхода для страницы «Партнёрки Директ» (без записи в БД): помесячно,
// по кампаниям и по провайдерам, только «в бюджете». Для кампаний без строки настроек
// применяем guessDefaults в памяти, чтобы не зависеть от визита на /projects.
export async function loadSpendSummary() {
  const m = loadMarketing();
  const spendDaily = (m && m.spendDaily && typeof m.spendDaily === 'object') ? m.spendDaily : {};
  const snapCamps = (m && m.direct && Array.isArray(m.direct.campaigns)) ? m.direct.campaigns : [];
  const nameOf = {}; const cumCost = {};
  for (const [id, node] of Object.entries(spendDaily)) {
    cumCost[id] = Object.values((node && node.days) || {}).reduce((s, v) => s + num(v), 0);
    if (node && node.name) nameOf[id] = node.name;
  }
  for (const c of snapCamps) { const id = String(c.id); if (!(id in cumCost)) cumCost[id] = num(c.cost); if (!nameOf[id]) nameOf[id] = c.name || ''; }

  const projects = await getProjects();
  const provSet = new Set(projects.filter((p) => p.kind === 'provider').map((p) => p.slug));
  const siteSet = new Set(projects.filter((p) => p.kind === 'site').map((p) => p.slug));
  const rows = await q('select campaign_id, status, in_budget, provider_alloc, site_alloc, alloc from ad_campaign_settings');
  const settingsMap = {};
  for (const r of rows) settingsMap[r.campaign_id] = { ...r, alloc: r.alloc || resolveAlloc(r.provider_alloc, r.site_alloc, provSet, siteSet) };
  for (const id of Object.keys(cumCost)) {
    if (settingsMap[id]) continue;
    const d = guessDefaults({ id, name: nameOf[id] || '' });
    settingsMap[id] = { status: d.status, in_budget: d.in_budget, alloc: d.alloc };
  }
  const inBudget = (id) => { const s = settingsMap[id]; return s && s.status !== 'excluded' && s.in_budget; };
  const byMonth = monthlySpend(spendDaily, inBudget);
  const attr = attribute(Object.keys(cumCost).map((id) => ({ id, cost: cumCost[id] })), settingsMap, projects);

  // помесячная разбивка ПО ПРОВАЙДЕРАМ (для столбцов с накоплением + подсказки)
  const PROV_COLORS = { timeweb: '#c6a15b', adminvps: '#5b7a99', aeza: '#b98cc4', ishosting: '#6cbf8b', hostman: '#6e56cf' };
  const provList = projects.filter((p) => p.kind === 'provider' && !p.archived);
  const provSlugs = provList.map((p) => p.slug);
  const nProv = provSlugs.length || 1;
  const moProv = {};
  for (const [id, node] of Object.entries(spendDaily)) {
    if (!inBudget(id)) continue;
    const s = settingsMap[id]; const a = s && s.alloc;
    const toProv = provSet.has(a) ? [a] : provSlugs;
    const share = provSet.has(a) ? 1 : 1 / nProv;
    for (const [d, cost] of Object.entries((node && node.days) || {})) {
      const mo = String(d).slice(0, 7);
      const bucket = moProv[mo] || (moProv[mo] = {});
      for (const ps of toProv) bucket[ps] = (bucket[ps] || 0) + num(cost) * share;
    }
  }
  const byMonthProv = Object.keys(moProv).sort().map((mo) => {
    const prov = {}; let total = 0;
    for (const ps of provSlugs) { const c = Math.round(moProv[mo][ps] || 0); prov[ps] = c; total += c; }
    return { m: mo, prov, total };
  });
  const providers = provList.map((p) => ({ slug: p.slug, name: p.name, color: PROV_COLORS[p.slug] || '#7f9dbb' }));
  const byCampaign = Object.keys(cumCost)
    .filter(inBudget)
    .map((id) => ({ id, name: nameOf[id] || id, cost: Math.round(cumCost[id]) }))
    .filter((r) => r.cost > 0)
    .sort((a, b) => b.cost - a.cost);
  const net = projects.filter((p) => p.kind === 'provider' && !p.archived && p.roy_key).map((p) => {
    const revenue = revenueFor(p.roy_key); const spend = attr.spendProv[p.slug] || 0;
    return { slug: p.slug, name: p.name, revenue, spend, net: revenue == null ? null : revenue - spend };
  });
  return { total: Math.round(attr.counted), byMonth, byMonthProv, providers, byCampaign, spendProv: attr.spendProv, net, since: (m && m.spendSince) || '2026-02-01' };
}

// Сохранение правок из кабинета. projects: [{id?,slug,name,kind,roy_key,archived,sort}],
// deletedProjects: [id], settings: [{campaign_id,status,in_budget,provider_alloc,site_alloc}],
// deletedCampaigns: [campaign_id].
export async function saveBoard(payload = {}) {
  const { projects = [], deletedProjects = [], settings = [], deletedCampaigns = [] } = payload;
  for (const p of projects) {
    if (p.id) {
      await q(
        'update ad_projects set name=$2, kind=$3, roy_key=$4, archived=$5, sort=$6 where id=$1',
        [p.id, String(p.name || ''), p.kind === 'site' ? 'site' : 'provider', String(p.roy_key || ''), Boolean(p.archived), num(p.sort)]
      );
    } else if (p.slug) {
      await q(
        `insert into ad_projects (slug, name, kind, roy_key, archived, sort)
         values ($1,$2,$3,$4,$5,$6) on conflict (slug) do update set name=excluded.name, kind=excluded.kind, roy_key=excluded.roy_key, archived=excluded.archived, sort=excluded.sort`,
        [String(p.slug), String(p.name || ''), p.kind === 'site' ? 'site' : 'provider', String(p.roy_key || ''), Boolean(p.archived), num(p.sort)]
      );
    }
  }
  for (const id of deletedProjects) await q('delete from ad_projects where id=$1', [id]);
  for (const s of settings) {
    const status = ['active', 'archived', 'excluded'].includes(s.status) ? s.status : 'active';
    await q(
      `insert into ad_campaign_settings (campaign_id, status, in_budget, alloc, updated_at)
       values ($1,$2,$3,$4, now())
       on conflict (campaign_id) do update set status=excluded.status, in_budget=excluded.in_budget,
         alloc=excluded.alloc, updated_at=now()`,
      [String(s.campaign_id), status, Boolean(s.in_budget), String(s.alloc || 'split')]
    );
  }
  for (const id of deletedCampaigns) await q('delete from ad_campaign_settings where campaign_id=$1', [String(id)]);
  return { ok: true };
}

// Расход по дням за произвольный период, только по кампаниям «в бюджете» из
// кабинета. Нужен странице Директа: снимок коннектора отдаёт агрегат за 30 дней,
// а история по дням есть в spendDaily с 01.02.2026, поэтому график может слушаться
// периода из общей шапки. Конверсий в этой истории нет — они только в снимке.
// Настройки кампаний одним местом: строки из кабинета плюс умолчания для тех,
// кого пользователь ещё не трогал. Возвращает функцию «эту кампанию считаем?».
async function spendKeeper(spendDaily) {
  const projects = await getProjects();
  const provSet = new Set(projects.filter((p) => p.kind === 'provider').map((p) => p.slug));
  const siteSet = new Set(projects.filter((p) => p.kind === 'site').map((p) => p.slug));
  const rows = await q('select campaign_id, status, in_budget, provider_alloc, site_alloc, alloc from ad_campaign_settings');
  const settings = {};
  for (const r of rows) settings[r.campaign_id] = { ...r, alloc: r.alloc || resolveAlloc(r.provider_alloc, r.site_alloc, provSet, siteSet) };
  for (const [id, node] of Object.entries(spendDaily)) {
    if (settings[id]) continue;
    const d = guessDefaults({ id, name: (node && node.name) || '' });
    settings[id] = { status: d.status, in_budget: d.in_budget, alloc: d.alloc };
  }
  return { settings, keep: (id) => { const s = settings[id]; return Boolean(s && s.status !== 'excluded' && s.in_budget); } };
}

/**
 * Всё, что «Директу» нужно по периоду, за один запрос настроек: дни выбранного
 * периода, дни предыдущего такого же отрезка и разбивка по кампаниям за период.
 * Отбор один и тот же — только кампании с галкой «в бюджете», поэтому итоги,
 * график и таблица кампаний на странице сходятся между собой.
 */
export async function spendPeriod(from, to, prevFrom) {
  const m = loadMarketing();
  const spendDaily = (m && m.spendDaily && typeof m.spendDaily === 'object') ? m.spendDaily : {};
  const { keep } = await spendKeeper(spendDaily);
  const start = prevFrom && prevFrom < from ? prevFrom : from;

  const byDay = {};
  const camps = [];
  for (const [id, node] of Object.entries(spendDaily)) {
    if (!keep(id)) continue;
    const days = (node && node.days) || {};
    const cl = (node && node.clicks) || {};
    const im = (node && node.impr) || {};
    const cv = (node && node.conv) || {};
    let cost = 0, clicks = 0, impressions = 0, conv = 0, hasConv = false, nDays = 0;
    for (const [d, c] of Object.entries(days)) {
      if (d < start || (to && d > to)) continue;
      const e = byDay[d] || (byDay[d] = { date: d, cost: 0, clicks: 0, impressions: 0, conv: 0, hasConv: false });
      e.cost += num(c); e.clicks += num(cl[d]); e.impressions += num(im[d]);
      if (cv[d] != null) { e.conv += num(cv[d]); e.hasConv = true; }
      // в разбивку по кампаниям идёт только выбранный период, без прошлого
      if (d < from) continue;
      cost += num(c); clicks += num(cl[d]); impressions += num(im[d]); nDays += 1;
      if (cv[d] != null) { conv += num(cv[d]); hasConv = true; }
    }
    if (nDays) {
      camps.push({
        id, name: (node && node.name) || id, days: nDays,
        cost: Math.round(cost), clicks: Math.round(clicks), impressions: Math.round(impressions),
        conversions: hasConv ? Math.round(conv) : 0,
        ctr: impressions ? Math.round((clicks / impressions) * 1000) / 10 : 0,
        cpc: clicks ? Math.round(cost / clicks) : 0,
      });
    }
  }
  const asRow = (d) => ({
    date: d, cost: Math.round(byDay[d].cost), clicks: Math.round(byDay[d].clicks),
    impressions: Math.round(byDay[d].impressions),
    conversions: byDay[d].hasConv ? Math.round(byDay[d].conv) : null,
  });
  const all = Object.keys(byDay).sort();
  return {
    days: all.filter((d) => d >= from).map(asRow),
    prevDays: all.filter((d) => d < from).map(asRow),
    campaigns: camps.sort((a, b) => b.cost - a.cost),
  };
}

export async function spendDaysRange(from, to) {
  const m = loadMarketing();
  const spendDaily = (m && m.spendDaily && typeof m.spendDaily === 'object') ? m.spendDaily : {};
  const projects = await getProjects();
  const provSet = new Set(projects.filter((p) => p.kind === 'provider').map((p) => p.slug));
  const siteSet = new Set(projects.filter((p) => p.kind === 'site').map((p) => p.slug));
  const rows = await q('select campaign_id, status, in_budget, provider_alloc, site_alloc, alloc from ad_campaign_settings');
  const settings = {};
  for (const r of rows) settings[r.campaign_id] = { ...r, alloc: r.alloc || resolveAlloc(r.provider_alloc, r.site_alloc, provSet, siteSet) };
  for (const [id, node] of Object.entries(spendDaily)) {
    if (settings[id]) continue;
    const d = guessDefaults({ id, name: (node && node.name) || '' });
    settings[id] = { status: d.status, in_budget: d.in_budget, alloc: d.alloc };
  }
  const keep = (id) => { const s = settings[id]; return Boolean(s && s.status !== 'excluded' && s.in_budget); };

  const byDay = {};
  for (const [id, node] of Object.entries(spendDaily)) {
    if (!keep(id)) continue;
    const days = (node && node.days) || {};
    const cl = (node && node.clicks) || {};
    const im = (node && node.impr) || {};
    const cv = (node && node.conv) || {};
    for (const [d, cost] of Object.entries(days)) {
      if (from && d < from) continue;
      if (to && d > to) continue;
      const e = byDay[d] || (byDay[d] = { date: d, cost: 0, clicks: 0, impressions: 0, conv: 0, hasConv: false });
      e.cost += num(cost); e.clicks += num(cl[d]); e.impressions += num(im[d]);
      if (cv[d] != null) { e.conv += num(cv[d]); e.hasConv = true; }
    }
  }
  return Object.keys(byDay).sort().map((d) => ({
    date: d,
    cost: Math.round(byDay[d].cost),
    clicks: Math.round(byDay[d].clicks),
    impressions: Math.round(byDay[d].impressions),
    // конверсии по дням появились в снимке позже расхода: null = за этот день их нет
    conversions: byDay[d].hasConv ? Math.round(byDay[d].conv) : null,
  }));
}

// ---------------------------------------------------------------------------
// Выгрузка ИСТИННОГО расхода для внешних потребителей (сборка роялти на Маке).
// Источник дней — spendDaily из снимка коннектора, источник правды «что считаем» —
// кабинет «Проекты и кампании» (таблица ad_campaign_settings). Кампании без строки
// настроек добираются guessDefaults в памяти, как и на странице. Возвращаем помесячно
// и итогом: расход, клики, показы + разрез по провайдерам и сайтам по той же логике
// разнесения, что и в кабинете (привязка к провайдеру/сайту, прочее — поровну).
// ---------------------------------------------------------------------------
export async function spendExport() {
  const m = loadMarketing();
  const spendDaily = (m && m.spendDaily && typeof m.spendDaily === 'object') ? m.spendDaily : {};
  const snapCamps = (m && m.direct && Array.isArray(m.direct.campaigns)) ? m.direct.campaigns : [];

  const nameOf = {};
  for (const [id, node] of Object.entries(spendDaily)) if (node && node.name) nameOf[id] = node.name;
  for (const c of snapCamps) if (!nameOf[String(c.id)]) nameOf[String(c.id)] = c.name || '';

  const projects = await getProjects();
  const provList = projects.filter((p) => p.kind === 'provider' && !p.archived);
  const siteList = projects.filter((p) => p.kind === 'site' && !p.archived);
  const provSet = new Set(provList.map((p) => p.slug));
  const siteSet = new Set(siteList.map((p) => p.slug));
  const provSlugs = provList.map((p) => p.slug);
  const siteSlugs = siteList.map((p) => p.slug);

  const rows = await q('select campaign_id, name, status, in_budget, provider_alloc, site_alloc, alloc from ad_campaign_settings');
  const settingsMap = {};
  for (const r of rows) settingsMap[r.campaign_id] = { ...r, alloc: r.alloc || resolveAlloc(r.provider_alloc, r.site_alloc, provSet, siteSet) };
  for (const id of Object.keys(spendDaily)) {
    if (settingsMap[id]) continue;
    const d = guessDefaults({ id, name: nameOf[id] || '' });
    settingsMap[id] = { campaign_id: id, status: d.status, in_budget: d.in_budget, alloc: d.alloc, guessed: true };
  }
  const counts = (id) => { const s = settingsMap[id]; return Boolean(s && s.status !== 'excluded' && s.in_budget); };

  const blank = () => {
    const o = { cost: 0, clicks: 0, impr: 0, prov: {}, site: {} };
    for (const p of provSlugs) o.prov[p] = 0;
    for (const p of siteSlugs) o.site[p] = 0;
    return o;
  };
  const months = {};
  const total = blank();
  let costAll = 0; let costExcluded = 0;
  const campaigns = [];

  for (const [id, node] of Object.entries(spendDaily)) {
    const days = (node && node.days) || {};
    const cl = (node && node.clicks) || {};
    const im = (node && node.impr) || {};
    const s = settingsMap[id] || {};
    let cCost = 0; let cClicks = 0; let cImpr = 0;
    for (const [d, cost] of Object.entries(days)) {
      cCost += num(cost); cClicks += num(cl[d]); cImpr += num(im[d]);
    }
    costAll += cCost;
    if (s.status === 'excluded') costExcluded += cCost;
    campaigns.push({
      id, name: nameOf[id] || id, cost: Math.round(cCost), clicks: cClicks, impr: cImpr,
      status: s.status || 'active', in_budget: Boolean(s.in_budget), alloc: s.alloc || 'split',
      guessed: Boolean(s.guessed), counted: counts(id),
    });
    if (!counts(id)) continue;

    // разнесение: привязка к провайдеру → весь расход ему; к сайту → сайту;
    // всё остальное (и «поровну») делится на всех провайдеров / все сайты
    const a = s.alloc;
    const toProv = provSet.has(a) ? [a] : provSlugs;
    const shProv = provSet.has(a) ? 1 : (provSlugs.length ? 1 / provSlugs.length : 0);
    const toSite = siteSet.has(a) ? [a] : siteSlugs;
    const shSite = siteSet.has(a) ? 1 : (siteSlugs.length ? 1 / siteSlugs.length : 0);

    for (const [d, cost] of Object.entries(days)) {
      const k = String(d).slice(0, 7);
      const mo = months[k] || (months[k] = blank());
      const c = num(cost); const kk = num(cl[d]); const ii = num(im[d]);
      mo.cost += c; mo.clicks += kk; mo.impr += ii;
      total.cost += c; total.clicks += kk; total.impr += ii;
      for (const ps of toProv) { mo.prov[ps] += c * shProv; total.prov[ps] += c * shProv; }
      for (const ss of toSite) { mo.site[ss] += c * shSite; total.site[ss] += c * shSite; }
    }
  }

  const round = (o) => ({
    cost: Math.round(o.cost), clicks: Math.round(o.clicks), impr: Math.round(o.impr),
    prov: Object.fromEntries(Object.entries(o.prov).map(([k, v]) => [k, Math.round(v)])),
    site: Object.fromEntries(Object.entries(o.site).map(([k, v]) => [k, Math.round(v)])),
  });

  return {
    since: (m && m.spendSince) || '2026-02-01',
    generated: (m && m.generated) || null,
    asof: new Date().toISOString().slice(0, 10),
    total: { ...round(total), cost_all: Math.round(costAll), cost_excluded: Math.round(costExcluded) },
    months: Object.keys(months).sort().map((k) => ({ m: k, ...round(months[k]) })),
    providers: provList.map((p) => ({ slug: p.slug, name: p.name, roy_key: p.roy_key || null })),
    sites: siteList.map((p) => ({ slug: p.slug, name: p.name })),
    campaigns: campaigns.sort((a, b) => b.cost - a.cost),
  };
}
