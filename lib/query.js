...
// (реферер без рекламных признаков), наши же площадки, соцсети, прочее.
// Отличаем рекламный заход от органического так: у рекламы есть click-id
// (yclid/gclid и т.п.) или платный utm_medium (cpc/ppc/paid); органика это
// переход из поисковика БЕЗ этих признаков — значит нас нашли в выдаче.
const OUR_SITES = "(referrer_host like '%podborvps%' or referrer_host like '%servercalc%' or referrer_host like '%serverselection%')";
const SRC_CHANNEL = `case
    ...
    when ${OUR_SITES} then 'Наши сайты · переходы между площадками'
    ...
    when utm_source <> '' then 'Метки: ' || utm_source
    when referrer_host <> '' then 'Другие сайты'
    else 'Прямые заходы' end`;

// В канальном отчёте убираем только настоящий служебный шум (Метрика, Vercel,
// тестовый домен), а НАШИ контент-сайты оставляем — их показываем отдельным
// каналом «Наши сайты», чтобы были видны переходы между площадками.
const NOISE_HOSTS = [
  'metrika.yandex.ru',
  'vercel.com', 'vps-analytics.vercel.app',
  'dashboard.makebiztehnologies.com', 'makebiztehnologies.com', 'www.makebiztehnologies.com',
];
function noiseExcl(col) {
  const list = NOISE_HOSTS.map((h) => `'${h}'`).join(', ');
  return `${col} not in (${list}) and ${col} not like '%.vercel.app'`;
}
const W_SE_CH = `started_at >= ($1::timestamp at time zone $2)
  and started_at <  ($3::timestamp at time zone $2)
  and ($4::int is null or site_id = $4)
  and ($5::bool or is_bot = false)
  and ${noiseExcl('referrer_host')}`;
...
export async function logRows(f, opts = {}) {
  ...
  const rows = await q(
    `select e.id, e.ts, s.name site, s.key site_key, e.type, e.name, e.path, e.provider, e.target_host,
            ...
            e.click_id, e.country, e.city, e.device, e.browser, e.session_id, e.visitor_id, e.is_bot, e.meta
       from events e join sites s on s.id = e.site_id
      where ${sql}
...
export async function logAll(f, opts = {}) {
  ...
  return q(
    `select e.ts, s.name site, e.type, e.name, e.path, e.provider, e.target_host, e.target_url,
            ...
            e.click_id, e.country, e.city, e.device, e.browser, e.session_id
       from events e join sites s on s.id = e.site_id
