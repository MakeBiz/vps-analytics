...
// Воронки: считаем уникальные сессии по именованным событиям на каждом сайте.
// Шаги калькулятора (calc_start/calc_result/calc_click) и акций (promo_copy/promo_click)
// приходят как кастомные события 'ev' от пикселя сайта. Данные копятся с момента,
// когда на сайтах включили отправку этих событий в пиксель.
export async function funnelEvents(f) {
  const p = await baseParams(f);
  return q(
    `select s.key site_key, e.name ev, count(distinct e.session_id)::int sessions
       from events e join sites s on s.id = e.site_id
      where e.type = 'ev'
        and e.name in ('calc_start','calc_result','calc_click','promo_copy','promo_click')
