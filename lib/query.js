...
// Прокси-трафик Павлодара (KZ) — это наши тесты через казахстанский прокси,
// а не живые люди. Исключаем из всех отчётов, чтобы не искажал аналитику.
// Сами данные в базе остаются, просто не попадают в цифры.
const EXCL_PROXY = `not (country = 'KZ' and city = 'Pavlodar')`;
const EXCL_PROXY_E = `not (e.country = 'KZ' and e.city = 'Pavlodar')`;
...
export async function organicReport(f) {
  ...
  const byEngineSite = await q(
    `select s.key site_key, (${ORG_ENGINE}) engine,
            ...
            coalesce(sum(se.outclicks),0)::int clicks
       from sessions se join sites s on s.id = se.site_id
      where ${W_SE} and s.archived = false and ${ORG_WHERE}
      group by 1,2`,
    p
  );
  const byDay = await q(
    `select to_char(g.d,'YYYY-MM-DD') d,
       ...
       left join (
         select (started_at at time zone $2)::date d, (${ORG_ENGINE}) engine, count(*) visits
           from sessions where ${W_SE} and ${ORG_WHERE} group by 1,2
       ) v on v.d = g.d
  ...
  const pages = await q(
    `select s.key site_key, se.landing_path path,
       ...
       from sessions se join sites s on s.id = se.site_id
      where ${W_SE} and s.archived = false and se.landing_path <> '' and ${ORG_WHERE}
      group by 1,2 order by visits desc limit 80`,
    p
  );
  const cities = await q(
    `select city, country, count(*)::int visits, coalesce(sum(outclicks),0)::int clicks
       from sessions where ${W_SE} and ${ORG_WHERE} and city <> ''
      group by 1,2 order by visits desc limit 15`,
    p
  );
  const countries = await q(
    `select country, count(*)::int visits, coalesce(sum(outclicks),0)::int clicks
       from sessions where ${W_SE} and ${ORG_WHERE} and country <> ''
      group by 1 order by visits desc limit 10`,
    p
  );
  const devices = await q(
    `select device k, count(*)::int visits from sessions
      where ${W_SE} and ${ORG_WHERE} group by 1 order by visits desc limit 12`,
    p
  );
  const oss = await q(
