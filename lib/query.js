...
export async function geoReport(f) {
  ...
  const cities = await q(
    `select city, country, count(*)::int visits, coalesce(sum(outclicks),0)::int clicks
       from sessions where ${W_SE} and city <> '' and ${HIDE_PROXY_CITY} group by 1,2 order by visits desc limit 10`,
    p
  );
...
export async function organicReport(f) {
  ...
  const cities = await q(
    `select city, country, count(*)::int visits, coalesce(sum(outclicks),0)::int clicks
       from sessions where ${W_SE} and ${ORG_WHERE} and city <> '' and ${HIDE_PROXY_CITY}
