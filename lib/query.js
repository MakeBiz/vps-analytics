...
// Какие площадки попадают в отчёт: выбранный в шапке сайт, иначе все сайты
// выбранного направления, иначе (когда не выбрано ничего) все. null = без фильтра.
// Возвращаем массив id, чтобы номера bind-параметров не сдвигались.
async function siteIds(f) {
  if (f?.site) {
    const r = await q('select id from sites where key = $1', [f.site]);
    return [r[0]?.id ?? -1];
  }
  if (f?.line) {
    const r = await q(`select id from sites where line = $1 and ${NOT_DEAD}`, [f.line]);
    return r.length ? r.map((x) => x.id) : [-1];
  }
  return null;
}

export async function baseParams(f) {
  return [f.from, f.tz, f.toExclusive, await siteIds(f), f.bots];
}
...
// $1 начало периода, $2 таймзона, $3 конец (не включая), $4 массив id площадок
// (null = все), $5 показывать ли роботов
const W_EV = `ts >= ($1::timestamp at time zone $2)
  and ts <  ($3::timestamp at time zone $2)
  and ($4::int[] is null or site_id = any($4))
...
const W_EV_E = `e.ts >= ($1::timestamp at time zone $2)
  ...
  and ($4::int[] is null or e.site_id = any($4))
...
export async function sites() {
  return q(`select id, key, name, domain, archived, line from sites where ${NOT_DEAD} order by archived, id`);
}
...
export async function byDayBySite(f) {
  const p = await baseParams(f);
  return q(
    // посетители и просмотры нужны спарклайнам в карточках «Обзора»:
    // отдельный запрос ради двух колонок заводить незачем
    `select to_char(g.d,'YYYY-MM-DD') d, s.key site_key,
            coalesce(t.visits,0)::int visits, coalesce(t.clicks,0)::int clicks,
            coalesce(t.visitors,0)::int visitors, coalesce(t.pv,0)::int pv
       from generate_series($1::date, ($3::date - interval '1 day'), interval '1 day') g(d)
       cross join (select id, key from sites where archived = false and ${NOT_DEAD}) s
       left join (
         select d, site_id, sum(visits)::int visits, sum(clicks)::int clicks,
                sum(visitors)::int visitors, sum(pv)::int pv from (
           select (started_at at time zone $2)::date d, site_id, count(*) visits, 0 clicks,
                  count(distinct visitor_id) visitors, coalesce(sum(pageviews),0) pv
             from sessions where ${wSE(f)} group by 1,2
           union all
           select (ts at time zone $2)::date, site_id, 0, count(*) filter (where type = 'out'), 0, 0
...
// Заход без реферера и без рекламных меток — это тоже органика (клиент нашёл сайт
// в поиске и пришёл напрямую/из приложения без реферера): относим его к движку по
// сайту — servercalc.com (ключ *-com) к Google, русские сайты к Яндексу. Поэтому
// отдельного канала «без источника» больше нет — для этого нужен join c sites.
const SRC_CHANNEL = `case
    ...
    when sites.key like '%-com%' then 'Органика · Google'
    else 'Органика · Яндекс' end`;
...
// ---- Органика и SEO ----
// Движок органики по реферреру. Пустой реферер (заход «браузером», без сигнала об
// источнике) — это НЕ реклама и НЕ переход с чужого сайта, значит органика без
// известного движка ('none'): человек нашёл нас в поиске, но браузер/поисковик
// реферер не передал. Внешний непоисковый реферер (соцсети, чужие сайты) органикой НЕ считаем.
const ORG_ENGINE = `case
    ...
    when referrer_host = '' then 'none'
    else '' end`;
// Органика = заход из поиска ИЛИ прямой заход без реферера, БЕЗ рекламных признаков
// (без click-id рекламы и без платной utm-метки). Рекламные заходы с yclid/gclid сюда не попадают.
...
export async function organicReport(f) {
  ...
  // Динамика органики по дням в разрезе САЙТОВ (линия на сайт), а не движков.
  const byDay = await q(
    `select to_char(g.d,'YYYY-MM-DD') d, s.key site_key, coalesce(v.visits,0)::int visits
       from generate_series($1::date, ($3::date - interval '1 day'), interval '1 day') g(d)
       cross join (select id, key from sites where archived = false and ${NOT_DEAD}) s
       left join (
         select (started_at at time zone $2)::date d, site_id, count(*) visits
           from sessions where ${wSE(f)} and ${ORG_WHERE} group by 1,2
       ) v on v.d = g.d and v.site_id = s.id
      order by g.d, s.key`,
    p
  );
  const pages = await q(
